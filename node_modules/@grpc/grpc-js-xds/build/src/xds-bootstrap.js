...
exports.loadBootstrapInfo = exports.validateBootstrapConfig = exports.validateXdsServerConfig = exports.serverConfigEqual = void 0;
const fs = require("fs");
const environment_1 = require("./environment");
const KNOWN_SERVER_FEATURES = ['ignore_resource_deletion'];
function serverConfigEqual(config1, config2) {
...
exports.serverConfigEqual = serverConfigEqual;
...
function validateXdsServerConfig(obj) {
    ...
    if (!foundSupported) {
        throw new Error(`xds_servers.channel_creds field: must contain at least one entry with a type in [${SUPPORTED_CHANNEL_CREDS_TYPES}]`);
    }
    if ('server_features' in obj) {
        if (!Array.isArray(obj.server_features)) {
            throw new Error(`xds_servers.server_features field: expected array, got ${typeof obj.server_features}`);
        }
    ...
    return {
        ...
        serverFeatures: (_a = obj.server_features) !== null && _a !== void 0 ? _a : []
    };
}
exports.validateXdsServerConfig = validateXdsServerConfig;
...
function validateAuthority(obj, authorityName) {
    var _a, _b;
    ...
    return {
        clientListenerResourceNameTemplate: (_a = obj.client_listener_resource_name_template) !== null && _a !== void 0 ? _a : `xdstp://${authorityName}/envoy.config.listener.v3.Listener/%s`,
        xdsServers: (_b = obj.xds_servers) === null || _b === void 0 ? void 0 : _b.map(validateXdsServerConfig)
    };
}
function validateAuthoritiesMap(obj) {
    if (!obj) {
        return {};
    }
    const result = {};
...
function validateBootstrapConfig(obj) {
    var _a;
    ...
    if (environment_1.EXPERIMENTAL_FEDERATION) {
        ...
        return {
            ...
            clientDefaultListenerResourceNameTemplate: (_a = obj.client_default_listener_resource_name_template) !== null && _a !== void 0 ? _a : '%s'
        };
    }
    else {
        return {
...
function loadBootstrapInfo() {
    ...
    if (bootstrapConfig) {
        try {
            ...
            throw new Error(`Failed to parse xDS bootstrap config from environment variable GRPC_XDS_BOOTSTRAP_CONFIG with error ${e.message}`);
        }
        return loadedBootstrapInfo;
    }
    throw new Error('The GRPC_XDS_BOOTSTRAP or GRPC_XDS_BOOTSTRAP_CONFIG environment variables need to be set to the path to the bootstrap file to use xDS');
}
